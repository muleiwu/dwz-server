...
func (g *DistributedIDGenerator) GenerateID(domainID uint64) (uint64, error) {
	...
	if err != nil {
		logger.Logger().Error(fmt.Sprintf("Redis INCR失败: %v", err))
		return 0, err
	}
...
func (g *DistributedIDGenerator) InitializeDomainCounter(domainID uint64, startValue uint64) error {
	...
	if exists == 0 {
		...
		logger.Logger().Info(fmt.Sprintf("初始化域名%d计数器为%d", domainID, startValue))
	} else {
		...
		if uint64(current) < startValue {
			...
			logger.Logger().Info(fmt.Sprintf("更新域名%d计数器从%d到%d", domainID, current, startValue))
		}
	}
...
func (g *DistributedIDGenerator) GenerateShortCode(domainID uint64) (string, *uint64, error) {
	// 主方案：使用分布式发号器
	id, err := g.GenerateID(domainID)
	if err != nil {
...
func (g *DistributedIDGenerator) ResetDomainCounter(domainID uint64, newValue uint64) error {
	...
	logger.Logger().Warn(fmt.Sprintf("重置域名%d计数器为%d", domainID, newValue))
	return nil
}
